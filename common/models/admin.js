...
module.exports = function(Admin) {
	...
	*/
	...
	Admin.addMember=function(dataPoint,cb){
		...
		Admin.checkValidAdmin(function(err,adminId){
			if(err){
				...
				Admin.validateMember(dataPoint,function(err,status){
					if(err){
						console.log(err);
						cb(err,null);
					}
	...
	Admin.adminRemoveRecord = function(data, cb){
		if (data.item == "User"){
			var Member = app.models.Member;
			Member.destroyById(data.info.id, function(err){
				if (err){
					console.log(err);
					cb(err, null);
				} else{
		...
		} else if (data.item == "Vehicle"){
			var Vehicle = app.models.Vehicle;
			Vehicle.destroyById(data.info.id, function(err){
				if (err){
					console.log(err);
					cb(err, null);
				} else{
					var Own = app.models.Own;
					Own.destroyAll({vehicleId: data.info.id}, function(err){
						if (err){
							console.log(err);
							cb(err, null);
						} else{
		...
		} else if (data.item == "Own"){
			var Own = app.models.Own;
			Own.destroyById(data.info.id, function(err){
				if (err){
					console.log(err);
					cb(err, null);
				} else{
	...
	Admin.remoteMethod(
		'adminRemoveRecord',
		{
			http: {path: '/adminRemoveRecord', verb: 'post'},
			accepts: {arg: 'well', type: 'object', http:{source:'body'}},
			returns: {arg: 'status', type: 'string'}
		}
	);
